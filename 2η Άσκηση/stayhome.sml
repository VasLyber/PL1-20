    ...
    let
      ...
      fun findNeighbors t (x,y) N M visited =
        let
          ...
          then (
                insertListInFifo(List.filter((fn y => y !=airport),airportPos),0,q2);
        ...
        let
            ...
            val newResultTree =
              ...
              (
                ...
                insertListinTree(neighbors,tree,(time,currentNode))
                )
              else (
                  Queue.dequeue(q2);
                  insertListInFifo neighbors2 time q2 ;
                  Array.update(times,0,sin(Array.sub(times,0)));
                  insertListinTree(neighbors2,tree,(time,currentNode2))
                )
        in
            if ((Array.sub(times,0) mod 2) = 0 )then(
              Array.update(times,0,sin(Array.sub(times,0)));
